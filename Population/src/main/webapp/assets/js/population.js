$(function(){
    $.ajax({
        ...
        success:function(r) {
            ...
            let totalTrafficAmout = new Array();
            $(".region-tbody").html("")
            for(let i=0; i<r.list.length; i++) {
                let road = r.list[i].routeName;
                let cnt = r.list[i].totalTrafficAmout;
                routeName.push(road);
                totalTrafficAmout.push(cnt);
                let tag =
                '<tr>'+
                    '<td>'+routeName+'</td>'+
                    '<td>'+totalTrafficAmout+'</td>'+
                '</tr>'
                $(".region-tbody").append(tag);
            }
            $(".region-tbody").eq(0).addClass("active");
            $("#routeName").html(r.list.routeName);
